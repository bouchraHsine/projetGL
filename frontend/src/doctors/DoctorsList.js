            ...
            <div key={d._id} className="p-4 bg-white rounded shadow flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 rounded-full bg-gray-100 overflow-hidden flex items-center justify-center">
                  {d.photo ? (
                    <img src={(process.env.REACT_APP_API_BASE || "http://localhost:5000") + d.photo} alt={d.name} className="object-cover w-full h-full" />
                  ) : (
                    <FiUser size={28} />
                  )}
                </div>
              ...
              <div className="flex items-center gap-2">
                {/* Voir: utilise navigate pour forcer fonctionnement identique */}
